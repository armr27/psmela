...
if (empty($_SESSION['admin'])) {
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if (
            ...
            $_SESSION['errEmpty'] = 'ERROR! Semua form wajib diisi ada yang kurang';
                    ...
                    if (!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $asal_surat)) {
                        ...
                        if (!preg_match("/^[a-zA-Z0-9.,_()%&@\/\r\n -]*$/", $isi)) {
                            ...
                            if (!preg_match("/^[0-9.-]*$/", $tgl_surat)) {
                                ...
                                $cek = mysqli_query($config, "SELECT * FROM tbl_surat_masuk WHERE no_agenda='$no_agenda'");
                                $result = mysqli_num_rows($cek);

                                if ($result > 0) {
                                    $_SESSION['errDup'] = 'Nomor Agenda sudah ada, Silahkan Cek Kembali!!!';
                                    echo '<script language="javascript">window.history.back();</script>';
                                } else {
