    ...
    if ($_SESSION['admin'] != 1 and $_SESSION['admin'] != 2) {
        ...
        if (isset($_REQUEST['sub'])) {
            ...
            if (isset($_REQUEST['submit'])) {
                ...
                if (
                    $_REQUEST['instansi'] == "" || $_REQUEST['nama'] == "" || $_REQUEST['status'] == "" || $_REQUEST['alamat'] == "" || $_REQUEST['kapus'] == "" || $_REQUEST['nip'] == ""
                    ...
                    if (!preg_match("/^[a-zA-Z0-9. -]*$/", $nama)) {
                        ...
                        if (!preg_match("/^[a-zA-Z0-9. -]*$/", $instansi)) {
                            $_SESSION['instansi'] = 'Form Nama Instansi hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan minus(-)';
                            echo '<script language="javascript">window.history.back();</script>';
                        } else {
                            ...
                            if (!preg_match("/^[a-zA-Z0-9.,:\/<> -\"]*$/", $status)) {
                                ...
                                if (!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $alamat)) {
                                    ...
                                    if (!preg_match("/^[a-zA-Z., ]*$/", $kapus)) {
                                        $_SESSION['kapus'] = 'Form Nama Kepala Puskesmas hanya boleh mengandung karakter huruf, spasi, titik(.) dan koma(,)<br/><br/>';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {

                                        if (!preg_match("/^[0-9 -]*$/", $nip)) {
                                            $_SESSION['nipkapus'] = 'Form NIP Kepala Puskesmas hanya boleh mengandung karakter angka, spasi, dan minus(-)<br/><br/>';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {
                                                    ...
                                                    if (!empty($ttd)) {
                                                        ...
                                                        if (in_array($eks, $ekstensi) == true) {
                                                            if ($ukuran < 2000000) {
                                                                ...
                                                                $query = mysqli_query($config, "SELECT ttd FROM tbl_instansi");
                                                                list($ttd) = mysqli_fetch_array($query);

                                                                unlink($target_dir . $ttd);

                                                                move_uploaded_file($_FILES['ttd']['tmp_name'], $target_dir . $nttd);

                                                                $query = mysqli_query($config, "UPDATE tbl_instansi SET instansi='$instansi',nama='$nama',status='$status',alamat='$alamat',kapus='$kapus',nip='$nip',website='$website',email='$email',ttd='$nttd',id_user='$id_user' WHERE id_instansi='$id_instansi'");
                                                        ...
                                                        $query = mysqli_query($config, "UPDATE tbl_instansi SET instansi='$instansi',nama='$nama',status='$status',alamat='$alamat',kapus='$kapus',nip='$nip',website='$website',email='$email',id_user='$id_user' WHERE id_instansi='$id_instansi'");
                            ...
                            <form class="col s12" method="post" action="?page=sett" enctype="multipart/form-data">
                                ...
                                <div class="row">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <input id="instansi" type="text" class="validate" name="instansi" value="<?php echo $row['instansi']; ?>" required>
                                        <?php
                                        if (isset($_SESSION['instansi'])) {
                                            $instansi = $_SESSION['instansi'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $instansi . '</div>';
                                            unset($_SESSION['instansi']);
                                        }
                                        ?>
                                        <label for="instansi">Nama Dinas</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <input id="kapus" type="text" class="validate" name="kapus" value="<?php echo $row['kapus']; ?>" required>
                                        <?php
                                        if (isset($_SESSION['kapus'])) {
                                            $kapus = $_SESSION['kapus'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $kapus . '</div>';
                                            unset($_SESSION['kapus']);
                                        }
                                        ?>
                                        <label for="kapus">Nama Kepala Puskesmas</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        ...
                                        <?php
                                        if (isset($_SESSION['nipkapus'])) {
                                            $nipkapus = $_SESSION['nipkapus'];
                                            echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' . $nipkapus . '</div>';
                                            unset($_SESSION['nipkapus']);
                                        }
                                        ?>
                                        <label for="nip">NIP Kepala Puskesmas</label>
                                    </div>
                                    <div class="input-field col s6">
                                    ...
                                    <div class="input-field col s6">
                                        <img class="ttd" height="200px" width="200px" src="upload/<?php echo $row['ttd']; ?>" alt="Tidak ada File" />
                                    </div> -->
                                </div>
                                <!-- Row in form END -->
